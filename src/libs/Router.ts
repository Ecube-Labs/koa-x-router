 ...
 */
export class Router<StateT = any, CustomT = {}> extends KoaRouter<StateT, CustomT> {
    adaptors: Record<string, XRouterAdaptor>;

    constructor({ adaptors, ...pt }: KoaRouter.RouterOptions & { adaptors?: XRouterAdaptor[] } = {}) {
        super(pt);
    ...
    add(spec: RouteLayerSpec<StateT, CustomT> | RouteLayerSpec<StateT, CustomT>[]) {
    ...
    private getOASByStack(stack: KoaRouter.Layer[]) {
        ...
        stack.forEach((layer) => {
            if (layer.path && typeof layer.path === 'string' && layer.methods) {
                ...
                layer.methods.forEach((method) => {
                    if (layer.methods.length > 1 && method.toLowerCase() === 'head') return;
    ...
    private getOASRequestParameterSchema(layer: KoaRouter.Layer): ParameterObject[] | undefined {
        const { header, _headerAdaptor, query, _queryAdaptor, params, _paramsAdaptor } = layer.opts.validate || {};
    ...
    private getOASRequestBodySchema(layer: KoaRouter.Layer): RequestBodyObject | undefined {
    ...
    private registerValidator(layer: KoaRouter.Layer, validate?: XRouterValidateProperties) {
        if (validate) {
            layer.opts.validate = validate;
            if (layer.opts.validate.header) {
                layer.opts.validate._headerAdaptor = this.getAdaptorFromSchema(layer.opts.validate.header);
            }
            if (layer.opts.validate.query) {
                layer.opts.validate._queryAdaptor = this.getAdaptorFromSchema(layer.opts.validate.query);
            }
            if (layer.opts.validate.params) {
                layer.opts.validate._paramsAdaptor = this.getAdaptorFromSchema(layer.opts.validate.params);
            }
            if (layer.opts.validate.body) {
                layer.opts.validate._bodyAdaptor = this.getAdaptorFromSchema(layer.opts.validate.body);
            }
            if (layer.opts.validate.output) {
                const outputSchema = Object.values(layer.opts.validate.output)[0];
                if (outputSchema?.body) {
                    layer.opts.validate._outputAdaptor = this.getAdaptorFromSchema(outputSchema.body);
                }
            }
    ...
    private getAdaptorFromSchema(schema: any): XRouterAdaptor | undefined {
        const adaptor = Object.values(this.adaptors).find((adaptor) => adaptor.isCompatibleSchema(schema));
    ...
    private makeValidateMiddleware(layer: KoaRouter.Layer): KoaRouter.Middleware<any, any> {
        return async (ctx, next) => {
            ...
            const inputHandlerFactory = (reqPropName: 'headers' | 'params' | 'query' | 'body') => (values: any) => {
                if (values && Object.keys(values).length && ctx.request[reqPropName]) {
            ...
            await _headerAdaptor
                ...
                .catch(errorHandler);
            await _paramsAdaptor?.validate(params!, ctx.params).then(inputHandlerFactory('params')).catch(errorHandler);
            await _queryAdaptor
                ?.validate(query!, ctx.request.query)
                .then(inputHandlerFactory('query'))
                .catch(errorHandler);
            await _bodyAdaptor?.validate(body!, ctx.request.body).then(inputHandlerFactory('body')).catch(errorHandler);
            ...
            if (_outputAdaptor && output) {
                ...
                if (outputSchema?.body) {
                    ...
                    await _outputAdaptor.validate(outputSchema.body, ctx.body).then(outputHandler).catch(errorHandler);
                }
            }
        };
    }
